...
class TemplateManager:
    ...
    def load_templates(self, template_folder):
        # Carica i template solo se non sono già stati caricati
        if not self.templates:
            ...
            self.templates = {filename: cv2.imread(os.path.join(template_folder, filename), cv2.IMREAD_COLOR)
                              for filename in template_info.keys()}

    def get_template(self, filename):
        return self.templates.get(filename, None)
...
def worker_process(template_info_tuple, image, template_manager):
    ...
    if template is None:
        print(f"Error loading template: {filename}")
        return []
...
def main():
    ...
    template_manager.load_templates(template_folder)
    ...
    num_processes = max(1, min(multiprocessing.cpu_count(), len(template_info_list)))

    pool = multiprocessing.Pool(processes=num_processes)

    results = pool.starmap(worker_process, [(info, image, template_manager) for info in template_info_list])
